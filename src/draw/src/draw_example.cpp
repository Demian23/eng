...
public:
    An_Image_Window(int w, int h) : Fl_Window(w, h)
    {
        ...
        eng::floating xMin = 0, xMax = xMin + XSize, yMin = 0,
                      yMax = yMin + YSize, zMin = 0.1, zMax = 10;
        eng::vec::ThreeDimensionalVector eye{0, 0, 0},
            target{0, objPosition, 1}, up{0, -1, 0}, scale{40, 40, 1};
        // how to insert here division on w
        auto resultTransformation =
    ...
    virtual void draw()
    {
        ...
        for (const auto &vertex : vertices) {
            size_t index =
                (((size_t)vertex[1] * XSize + (size_t)vertex[0])) * 3;
            data[index] = 0;
            data[index + 1] = 0;
            data[index + 2] = 0;
        }
