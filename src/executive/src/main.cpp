...
std::string readArgsAndReturnPathToObj(int argc, const char* argv[],
                                       eng::ent::Model& modelForInit,
                                       eng::ent::Camera& cameraForInit,
                                       eng::ent::CameraProjection& projectionForInit,
                                       eng::ent::DistantLight& lightForInit,
                                       bool& isAutoPositioning);

void autoPositioning(eng::ent::Model& model, eng::ent::Camera &camera, eng::ent::CameraProjection &projection);
...
int main(int argc, const char *argv[])
{
    ...
    try{
        ...
        eng::ent::CameraProjection projection{static_cast<eng::floating>(w), static_cast<eng::floating>(h)};
        eng::ent::DistantLight light{};
        bool isAutoPositioning{false};

        auto path = readArgsAndReturnPathToObj(argc, argv, model, camera, projection, light, isAutoPositioning);
        ...
        ScreenDrawer drawer{w, h - 20, std::move(model), camera, projection, light};
        drawer.end();
        drawer.show();
        return Fl::run();
    } catch(...){
...
void autoPositioning(eng::ent::Model& model, eng::ent::Camera &camera, eng::ent::CameraProjection &projection)
{
    auto [xMin, xMax, yMin, yMax, zMin, zMax] = eng::alg::boundingBox<3>(model.verticesBegin(), model.verticesEnd());
    auto modelWidth = xMax - xMin;
    auto modelHeight = yMax - yMin;
    auto modelThickness = zMax - zMin;

    model.addModelTransformation(eng::mtr::Matrix::getMove({0, -modelHeight / 2, -modelThickness / 2}));
              ...
              bool& isAutoPositioning)
{
    ...
    options.add_options()
        ...
        ("c,camera", "Camera position vector in polar coordinates",
         cxxopts::value<std::vector<eng::floating>>()->
             default_value("3,0,0"))
        ...
        ("f,fov", "Fov angle in projection matrix", cxxopts::value<eng::floating>())
    ...
    auto vecToVec3F = [](const std::vector<eng::floating>& vec){return eng::vec::Vec3F{vec.at(0), vec.at(1), vec.at(2)};};
    lightForInit.color = vecToVec3F(result["light-color"].as<std::vector<eng::floating>>());
    modelForInit.setAlbedo(vecToVec3F(result["albedo"].as<std::vector<eng::floating>>()));
    if(result.count("fov"))
        projectionForInit.setAngleInDegrees(result["fov"].as<eng::floating>());

    isAutoPositioning = result["auto-positioning"].as<bool>();
    if(!isAutoPositioning){
        auto cameraEye {vecToVec3F(result["camera"].as<std::vector<eng::floating>>())};
        cameraEye[1] = eng::degreeToRadian(cameraEye[1]);
        cameraEye[2] = eng::degreeToRadian(cameraEye[2]);
        auto targetPosition{vecToVec3F(result["target"].as<std::vector<eng::floating>>())};
        cameraForInit.reset(cameraEye, targetPosition, {0, 1, 0});
        modelForInit.addModelTransformation(eng::mtr::Matrix::getScale(vecToVec3F(result["scale"].as<std::vector<eng::floating>>())));
    }
    return source;
}
...
void initModel(std::string_view pathToObj, eng::ent::Model& model)
{
    ...
    if (!std::filesystem::is_regular_file(pathToObj)) {
        throw std::logic_error(std::string(pathToObj) +
                               " is not regular file");
    }
    ...
    model.reset(std::move(vertices), std::move(triangles),
