...
public:
    ...
    [[nodiscard]] std::vector<PolygonType> applyTransformations() const noexcept
    {
        ...
        auto xMax = 0 + static_cast<floating>(w());
        auto yMax = 0 + static_cast<floating>(h());
        ...
        auto transformVertex = [transformationMatrix,
            ...
            *x = std::clamp(*x, static_cast<floating>(0), xMax);
            *y = std::clamp(*y, static_cast<floating>(0), yMax);
    ...
    void printOutStatistic(std::ostream &stream)
    {
        auto [cameraX, cameraY, cameraZ] =
            static_cast<std::array<eng::floating, 3>>(camera.getEye());
        auto [targetX, targetY, targetZ] =
            static_cast<std::array<eng::floating, 3>>(camera.getTarget());
        auto [diagonal, azimuth, polar] =
            static_cast<std::array<eng::floating, 3>>(
                eng::vec::cartesianToSpherical(camera.getEye()));
